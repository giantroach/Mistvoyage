...
- プレイヤーは周辺の海域のイベントを後述のSight値に応じて幾つか先まで見ることができます。
- イベントのタイプは以下の通りです。
    - 「???」: 何が起こるか分かりません。
...
## Sight
- 現在のSight値に応じてマップの幾つ先まで確認できるかが異なる。
   - Sight値 5以下: マップ上のイベントは全て「遠方のため不明」となり確認できない。
   - Sight値 6-10: 1つ先まで見ることができ、それ以外は「遠方のため不明」となる。
   - Sight値 11-15: 3つ先まで見ることができ、それ以外は「遠方のため不明」となる。
   - Sight値 16-20: 4つ先まで見ることができ、それ以外は「遠方のため不明」となる。
   - Sight値 21以上: 5つ先まで見ることができ、それ以外は「遠方のため不明」となる。
...
## 食料の消費
- 船はマップを移動するたびに食料を消費します。
    - 消費する食料は「乗組員の数 * 0.1 + 0.5」
- マップの移動の際、食料が足りない場合一定確立で乗組員が消費されます。
    - 乗務員が消費される確立は「足りない食料 * 10 %」です。
    - 乗組員が消費された場合、モーダルでその旨メッセージを表示します。
...
- 「雨」と「大雨」は「速度」「命中率」に悪影響を及ぼす。
     - 雨: 速度 -1, 命中率 -10%
     - 大雨: 速度 -2, 命中率 -20%
- 「霧」と「濃霧」は「命中率」と「Sight」に悪影響を及ぼす。
     - 霧: 命中率 -15%, Sight: -5
     - 濃霧: 命中率 -30%, Sight: -10
- 「嵐」は「速度」「命中率」「Sight」に悪影響を及ぼす。
     - 濃霧: 速度: -2, 命中率 -30%, Sight: -10

# 属性
- 各モンスターは一つ以上の「装甲タイプ」を持ち、各武器は「武器タイプ」を持つ。
- 装甲タイプに対する武器タイプの有効性は「battle_config.json」に定義される。
    - 有効性は%で指定され、0.5であれば半分しかダメージを与えられない事を意味し、2であれば2倍のダメージを与えらえることを意味する。
- 複数の装甲タイプをもつ場合、それぞれの倍率をかけ合わせる。
    - 例えばそれぞれの倍率が0.8と2の場合1.6となる。
## 武器タイプ
    - 貫通
    - 火炎
    - 衝撃
...
## 港
- 港では以下のサービスを任意の回数、使用することが出来る。
### 船体の修復
- 金と引き換えに船体を10修復する。必用な金額は各海域の定義ファイルに定義されている
### 武器の購入
- ランダムに提示される3つの武器から任意のものを任意の数、購入できる。
    - ただし最初に提示された3つ以外のもは提示されない。
    - 提示される武器のレアリティは chapters.json に海域ごとに定義される。
### レリックの購入
- ランダムに提示される3つのレリックから任意のものを任意の数、購入できる。
    - ただし最初に提示された3つ以外のもは提示されない。
    - 提示されるレリックのレアリティは chapters.json に海域ごとに定義される。
### 乗組員を雇う
- 金と引き換えに乗組員を1人雇う。乗組員が上限の場合これは使用できない。
### 食料を買う
- 金と引き換えに食料を5購入します。
- 金額は chapters.json にチャプターごとに定義される。
...
### モンスターの攻撃によるCrewの減少
- モンスターの攻撃にはcrewLoss (chance, min, max)が設定されている。
- モンスターの攻撃がヒットした際、crewLoss.chanceのチェックを行い、該当する場合はmin maxの範囲でcrewを失う。
    - crewLossのchanceやmin maxは主に小さい数字である。(2, 1, 1のように)
    - crewLoss.chanceはレリックの効果により低減できる事がある(chanceを50%低減のように)
...
## ???
- 「宝」「港」「寺院」「モンスター」「モンスター」のいずれかのイベントがランダムに発生する。
- 各イベントの発生確率は chapters.json に定義される。
